...
; generate (1, 2,...,8) -> map + accumulate -> adjoin-postition new-row 8-1 rest-of-queens -> filter out non-safe positions
(define (queens board-size)
  (define (queen-cols k)
    (if (= k 0)
        (list empty-board)
        (filter (lambda (postitions) (safe? k postitions))
                (flatmap (lambda (rest-of-queens)
                           (map
                            (lambda (new-row) (adjoin-position new-row
                                                               k
                                                               rest-of-queens))
                            (enumerate-interval 1 board-sze)))
                         (queen-cols (- k 1))))))
...
- typically describe a language by means of its primiatives, means of combination and abstractions
- this exercise only has one property: the *painter*
- transforms which take a single painter and produce new painter (e.g. flip-vert) highlight the data *closure* property
- can combine painters with Scheme procedures (so technically won't feature in the picture language)
